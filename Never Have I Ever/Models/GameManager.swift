...
class GameManager {
    ...
    var availableStatements: [Statement] = []
    var usedStatements: [Statement] = []
    var customStatements: [Statement] = []
    var currentStatement: Statement?
    var selectedCategories: Set<Statement.Category> = Set(Statement.Category.allCases.filter { $0 != .custom && $0 != .spicy })
    var isGameActive: Bool = false
    var useCustomOnly: Bool = false
    var totalStatementsPlayed: Int = 0
    var gamesCompleted: Int = 0
    var categoriesPlayed: Set<Statement.Category> = []
    ...
    func nextStatement() {
        ...
        if let current = currentStatement {
            ...
            // Check for night owl (midnight) / early bird (1–5 AM)
    ...
    /// Toggle custom-only mode and persist the preference
    func toggleUseCustomOnly() {
        useCustomOnly.toggle()
